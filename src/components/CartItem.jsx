...
const CartItem = ({item}) => {
  ...
  return (
    ...
    <div className='w-full min-h-[150px] flex items-center gap-x-4'>
      <div>
        <img className='max-w-[80px]' alt='itemImage' src={image}/>
      </div>
      <div className='w-full flex flex-col'>
        <div className='flex justify-between mb-2'>
          <p className='text-sm uppercase font-medium
            max-w-[240px] text-primary'>{title}
          </p>
        <div onClick={()=>removeFromCart(id)} className='text-xl cursor-pointer'>
          <IoMdClose className='text-gray-500
           hover:text-red-500 trasition'/>
        </div>
       </div>
       <div className='flex gap-x-2 text-sm h-[36px]'>
        {/* {qty} */}
        <div className='flex flex-1 max-w-[100px]
        items-center h-full text-primary font-medium border'>
